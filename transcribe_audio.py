...
def transcribe_audio_with_diarization(audio_file_path, assemblyai_api_key, num_speakers=None, force=False):
    ...
    if "transcript" in output_data and not force:
        print(f"Transcript data found in '{output_filename}'. Skipping transcription (use --force-transcribe to re-run).")
...
def summarize_transcript_with_xai(transcript_segments, audio_file_path, xai_api_key, model_id, audio_datetime_str=None, force=False, max_tokens=10000):
    ...
    if ("summary" in output_data or "xai_usage" in output_data) and not force:
        print(f"Summary or xAI usage data found in '{output_filename}'. Skipping summarization (use --force-summarize to re-run).")
    ...
    try:
        ...
        if summary:
            ...
            if speaker_name_mapping:
                ...
                if "transcript" in output_data and isinstance(output_data["transcript"], dict) and "segments" in output_data["transcript"] and isinstance(output_data["transcript"]["segments"], list):
                    ...
                    print("Warning: Transcript segments not found in output_data in expected format for name mapping update.")
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--speakers", type=int, default=3,
    ...
    transcript_segments = transcribe_audio_with_diarization(audio_file_path, assemblyai_api_key, expected_speakers, force=force_transcribe)
