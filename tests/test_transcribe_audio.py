...
class TestHelperFunctions(unittest.TestCase):
    ...
    def test_extract_speaker_name_mapping(self):
        ...
        expected2 = {"Speaker D": "Dr. Smith"}  # Updated expectation
    ...
    @mock.patch(
        "transcribe_audio_app.transcribe_audio.MutagenFile", create=True
    )  # create=True if MutagenFile might not exist
...
class TestDownloadFunction(unittest.TestCase):
    ...
    def test_download_success_basic_url(
        self, mock_os_makedirs, mock_open_builtin, mock_requests_get, mock_requests_head
    ):
    ...
    def test_download_timeout(self, mock_requests_head):
        mock_requests_head.side_effect = transcribe_audio.requests.exceptions.Timeout("Test timeout")
        result = transcribe_audio.download_file_from_url(
            "http://example.com/timeout.mp3", download_folder=self.DOWNLOAD_DIR
        )
        self.assertIsNone(result)
        self.mock_logger.error.assert_called_once()
        self.assertIn("Download timed out", self.mock_logger.error.call_args[0][0])

    @mock.patch("transcribe_audio_app.transcribe_audio.requests.head")
    def test_download_connection_error(self, mock_requests_head):
        mock_requests_head.side_effect = transcribe_audio.requests.exceptions.ConnectionError("Test connection error")
        result = transcribe_audio.download_file_from_url(
            "http://example.com/connection_error.mp3", download_folder=self.DOWNLOAD_DIR
        )
        self.assertIsNone(result)
        self.mock_logger.error.assert_called_once()
        self.assertIn("Connection error", self.mock_logger.error.call_args[0][0])

    @mock.patch("transcribe_audio_app.transcribe_audio.requests.head")
    def test_download_http_error_on_head(self, mock_requests_head):
        mock_head_response = mock.Mock()
        mock_head_response.raise_for_status.side_effect = transcribe_audio.requests.exceptions.HTTPError(
            "404 Client Error"
        )
        mock_requests_head.return_value = mock_head_response

        result = transcribe_audio.download_file_from_url(
            "http://example.com/notfound.mp3", download_folder=self.DOWNLOAD_DIR
        )
        self.assertIsNone(result)
        self.mock_logger.error.assert_called_once()
        self.assertIn("HTTP error", self.mock_logger.error.call_args[0][0])
    ...
    def test_download_http_error_on_get(self, mock_requests_get, mock_requests_head):
        ...
        mock_get_response = mock.Mock()
        mock_get_response.raise_for_status.side_effect = transcribe_audio.requests.exceptions.HTTPError(
            "500 Server Error"
        )
        mock_requests_get.return_value.__enter__.return_value = mock_get_response

        result = transcribe_audio.download_file_from_url(
            "http://example.com/servererror.mp3", download_folder=self.DOWNLOAD_DIR
        )
        self.assertIsNone(result)
        self.mock_logger.error.assert_called_once()
        self.assertIn("HTTP error", self.mock_logger.error.call_args[0][0])
    ...
    def test_download_filename_sanitization(self, mock_open_builtin, mock_requests_get, mock_requests_head):
        ...
        url = "http://example.com/a<b>c.mp3"  # Simplified problematic URL
        sanitized_name = "a_b_c.mp3"  # Corresponding expected sanitized name

        result_path = transcribe_audio.download_file_from_url(url, download_folder=self.DOWNLOAD_DIR)
        if result_path is None or not result_path.endswith(sanitized_name):  # Add this to print
            print(
                f"\n[DEBUG] In test_download_filename_sanitization:\n"
                f"      URL: {url}\n"
                f"      Expected sanitized_name: {sanitized_name}\n"
                f"      Actual result_path: {result_path}\n"
            )
        self.assertIsNotNone(result_path)
        actual_filename_in_path = os.path.basename(result_path)
        if actual_filename_in_path != sanitized_name:  # Debug print
            print(
                f"\n[DEBUG] In test_download_filename_sanitization (updated assertion):\n"
                f"      Expected sanitized_name (basename): {sanitized_name}\n"
                f"      Actual basename(result_path): {actual_filename_in_path}\n"
                f"      Full result_path: {result_path}\n"
            )
        self.assertEqual(actual_filename_in_path, sanitized_name)
...
class TestCoreProcessing(unittest.TestCase):
    ...
    # Adjusted DUMMY_OUTPUT_PATH to include './' to match actual usage in process_single_file
    DUMMY_OUTPUT_PATH = os.path.join(".", os.path.splitext(DUMMY_FILE_PATH)[0] + OUTPUT_JSON_SUFFIX)
    ...
    @mock.patch(
        "transcribe_audio_app.transcribe_audio.get_latest_xai_model", return_value="mock-grok-model"
    )  # Mock model selection
    @mock.patch("transcribe_audio_app.transcribe_audio.get_audio_duration", return_value=60.0)
    @mock.patch(
        "transcribe_audio_app.transcribe_audio.load_output_data", return_value={}
    ...
    def test_process_single_file_success(
        ...
        # Ensure the path used in the SUT matches the expectation
        # The SUT generates paths like "./filename_suffix.json" when the input is local to current dir
        mock_load_data.assert_called_with(self.DUMMY_OUTPUT_PATH)
    ...
    @mock.patch(
        "transcribe_audio_app.transcribe_audio.transcribe_audio_with_diarization", return_value=(None, None)
    )  # Transcription fails
    ...
    @mock.patch(
        "transcribe_audio_app.transcribe_audio.transcribe_audio_with_diarization",
        return_value=([{"speaker": "A", "text": "Hello"}], 60.0),
    )
    @mock.patch(
        "transcribe_audio_app.transcribe_audio.summarize_transcript_with_xai", return_value=(None, None)
    )  # Summarization fails
    @mock.patch("transcribe_audio_app.transcribe_audio.get_latest_xai_model", return_value="mock-grok-model")
    @mock.patch("transcribe_audio_app.transcribe_audio.get_audio_duration", return_value=60.0)
    @mock.patch("transcribe_audio_app.transcribe_audio.load_output_data", return_value={})
    @mock.patch("transcribe_audio_app.transcribe_audio.save_output_data")
    @mock.patch("os.path.getmtime", return_value=time.time())
    def test_process_single_file_summarization_fails(
        self,
        mock_getmtime,
        mock_save_data,
        mock_load_data,
    ...
    @mock.patch("transcribe_audio_app.transcribe_audio.save_output_data")
    @mock.patch("os.path.getmtime", return_value=time.time())
    def test_process_single_file_no_xai_model(
        self,
        mock_getmtime,
        mock_save_data,
        mock_load_data,
