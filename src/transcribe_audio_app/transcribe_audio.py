...
try:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "pip", "install", "tqdm"], capture_output=True, text=True, check=False
    )
    if result.returncode == 0:
        ...
        print(
            f"WARNING: tqdm installation failed. Progress bars will not be available. Error: {result.stderr}",
            file=sys.stderr,
        )
...
try:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "pip", "install", "mutagen"], capture_output=True, text=True, check=False
    )
    if result.returncode == 0:
        try:
            from mutagen import File as MutagenFile
            ...
            MutagenFile = None
            print(
                "WARNING: Mutagen installed but could not be imported. Duration detection might be affected.",
                file=sys.stderr,
            )
    else:
        MutagenFile = None
        print(
            f"WARNING: Mutagen installation failed. Duration detection might be affected. Error: {result.stderr}",
            file=sys.stderr,
        )
...
try:
    ...
    result = subprocess.run(
        [sys.executable, "-m", "pip", "install", "tinytag"], capture_output=True, text=True, check=False
    )
    if result.returncode == 0:
        try:
            from tinytag import TinyTag
            ...
            TinyTag = None
            print(
                "WARNING: Tinytag installed but could not be imported. Duration detection might be affected.",
                file=sys.stderr,
            )
    else:
        TinyTag = None
        print(
            f"WARNING: Tinytag installation failed. Duration detection might be affected. Error: {result.stderr}",
            file=sys.stderr,
        )
...
for handler in logging.getLogger("urllib3").handlers:
...
common_retry_settings = tenacity.retry(
    wait=tenacity.wait_exponential_jitter(max=60, jitter=10),
...
assemblyai_retry_settings = tenacity.retry(
    wait=tenacity.wait_exponential_jitter(max=60, jitter=10),
    stop=tenacity.stop_after_attempt(5),
    retry=(tenacity.retry_if_exception_type(requests.exceptions.RequestException) | retry_if_transient_http_error),
    before_sleep=tenacity.before_sleep_log(logger, logging.WARNING),  # Log retries at WARNING
    reraise=True,
)
        ...
        try:
            with open(filepath, "r") as f:
                ...
                if isinstance(loaded_data, dict):
                    ...
                    logger.warning(
                        f"Existing file '{filepath}' does not contain a JSON object. Starting with an empty structure."
                    )
...
def download_file_from_url(url, download_folder=DOWNLOAD_FOLDER, overall_pbar=None):
    ...
    try:
        ...
        file_extension = (
            file_extension if file_extension and file_extension.startswith(".") else ".download"
        ...
        if hasattr(e, "response") and e.response is not None:
            ...
            try:
                ...
                if (
                    ...
                    and hasattr(e.cause.response, "text")
                ):
...
def transcribe_audio_with_diarization(
    audio_file_path, assemblyai_api_key, num_speakers=None, force=False, overall_pbar=None, file_status_desc=""
):
    """
    ...
    try:
        ...
        def _transcribe_core(file_path, num_speakers_expected, pbar, file_desc):
            ...
            while transcript.status in ["queued", "processing"]:
                ...
                logger.debug(
                    f"Polling status for job {transcript.id}: {transcript.status.value}"
        ...
        transcript_object, aa_detected_duration = _transcribe_core(
            audio_file_path, num_speakers, overall_pbar, file_status_desc
        )
...
def extract_speaker_name_mapping(summary_text):
    ...
    name_mapping = {}
    # Regex to capture names, optionally including titles like "Dr. " or "Mr. ", etc.
    # It looks for (OptionalTitle FirstName LastName_or_Initial) (Speaker X)
    # or Speaker X (OptionalTitle FirstName LastName_or_Initial)
    name_pattern_text = r"((?:(?:Prof|Dr|Mr|Ms|Mrs)\.\s)?\b[A-Z][a-z]+(?:['\s-][A-Z][a-z]+)*|\b[A-Z]+\b)"

    pattern1 = re.compile(rf"{name_pattern_text}\s+\(Speaker ([A-Z]+)\)")
    ...
    pattern2 = re.compile(rf"Speaker ([A-Z]+)\s+\({name_pattern_text}\)")
    matches2 = pattern2.findall(summary_text)
    for speaker_label_suffix, name in matches2:
        original_label = f"Speaker {speaker_label_suffix}"
        name_mapping[original_label] = name.strip()

    # Keep a simplified version for "Name (Speaker X) said" as it's less likely to have complex titles there.
    ...
    for name, speaker_label_suffix in matches3:
        original_label = f"Speaker {speaker_label_suffix}"
        if original_label not in name_mapping:  # Prioritize more specific patterns if already matched
            name_mapping[original_label] = name.strip()

    logger.debug(f"Extracted potential speaker name mapping: {name_mapping}")
    return name_mapping
...
def process_single_file(
    ...
    file_status_prefix = (
        f"File {file_index}/{total_files}: {os.path.basename(file_path)}"
        if overall_pbar
    ...
    transcript_segments, aa_detected_duration = transcribe_audio_with_diarization(
        ...
        file_status_desc=file_status_prefix,
    )
    ...
    if overall_pbar:  # Check if pbar exists
        if duration_for_pbar_update is not None and duration_for_pbar_update > 0:  # Also check if duration is positive
            logger.debug(
                f"Updating overall_pbar with duration from {update_source}: {duration_for_pbar_update:.2f}"
            )  # Debug print
            overall_pbar.update(duration_for_pbar_update)
            overall_pbar.refresh()
        else:
            logger.debug(
                f"Valid duration ({duration_for_pbar_update}) from {update_source} not available, cannot update overall_pbar for {os.path.basename(file_path)}."
            )  # Debug print
            # Inform the user in non-verbose mode if duration update is skipped
            if logger.isEnabledFor(logging.INFO):
                if overall_pbar and not overall_pbar.disable:
                    overall_pbar.write(
                        f"Warning: Could not get valid duration ({update_source}) for {os.path.basename(file_path)}. Overall progress bar may not update accurately for this file."
                    )
                    overall_pbar.refresh()
    ...
    if transcript_segments is not None:
        ...
        if xai_api_key:  # Check if xAI key is available
            if selected_xai_model:  # Check if a model was successfully selected and passed
                ...
                logger.warning(
                    "A suitable xAI model was not provided or could not be determined. Skipping summarization."
                )  # Keep warning
        else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Transcribe and summarize audio/video from file, URL, or folder using AssemblyAI and xAI."
    )
    parser.add_argument(
        "input_source",
    ...
    if verbose_logging:
        ...
        for req_handler in logging.getLogger("requests").handlers:
            logging.getLogger("requests").removeHandler(req_handler)
        for url_handler in logging.getLogger("urllib3").handlers:
            logging.getLogger("urllib3").removeHandler(url_handler)
    ...
    if not assemblyai_api_key:
        logger.error(
            "AssemblyAI API key is required for transcription. Please set ASSEMBLYAI_API_KEY environment variable or provide it when prompted."
        )
        sys.exit(1)
    ...
    if input_source.lower().startswith(("http://", "https://")):
        ...
        if downloaded_file_path:
            ...
            if ext in SUPPORTED_EXTENSIONS:
                ...
                logger.warning(
                    f"Downloaded file format '{ext}' is not explicitly listed as supported by AssemblyAI. Skipping processing."
                )
        ...
        if logger.isEnabledFor(logging.INFO):
            ...
            logger.info(
                "Scanning folder for supported files and calculating total duration..."
            )  # Use logger info in verbose
            ...
            for file in files:
                ...
                if ext in SUPPORTED_EXTENSIONS:
                    ...
                    if duration is not None:
                        ...
                        logger.debug(
                            f"Added duration {duration:.2f}s for {os.path.basename(file_path)}. Current total: {total_duration:.2f}s"
                        )  # Debug only
                    else:
                        logger.debug(
                            f"Could not get duration for {os.path.basename(file_path)} upfront using mutagen/tinytag. Progress for this file will be best-effort."
                        )  # Debug only
                else:
        ...
        if unsupported_files_in_folder:
            logger.warning(
                f"\nSkipping {len(unsupported_files_in_folder)} files in the folder due to unsupported extensions:"
            )  # Keep warning
        ...
        if not supported_files_in_folder:
            logger.warning(
                f"No supported audio/video files found in the folder '{input_source}' or its subdirectories."
            )  # Keep warning
        else:
            ...
            if logger.isEnabledFor(logging.INFO):
                ...
                logger.info(
                    f"Found {len(supported_files_in_folder)} supported files in '{input_source}'."
                )  # Use logger info in verbose
            ...
            if total_duration > 0 and logger.isEnabledFor(logging.INFO):  # Check INFO level for pbar activation
                ...
                if logger.isEnabledFor(logging.INFO):
                    ...
                    logger.info(
                        f"Total detected audio duration: {format_duration(total_duration)}"
                    )  # Use logger info in verbose
            ...
            if xai_api_key:
                ...
                if total_duration == 0 and logger.isEnabledFor(logging.WARNING):  # Check WARNING level for message
                    logger.warning(
                        "Could not detect duration for any supported files upfront. Overall duration progress bar disabled."
                    )  # Keep warning
        ...
        if ext in SUPPORTED_EXTENSIONS:
            ...
            logger.warning(
                f"Local file format '{ext}' is not explicitly listed as supported by AssemblyAI. Skipping processing."
            )  # Keep warning
            logger.debug(f"Supported extensions include: {', '.join(sorted(SUPPORTED_EXTENSIONS))}")  # Debug only

    else:
        logger.error(
            f"Error: Input source '{input_source}' is not a valid URL, file path, or folder path."
        )  # Keep error
        parser.print_help()
        sys.exit(1)
